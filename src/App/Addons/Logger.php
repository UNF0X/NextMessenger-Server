...
class Logger
{
    ...
    static protected function getLogColor($level) {
        switch ($level) {
            ...
            case self::LEVEL_INFO: return "cyan";
    ...
    static protected function log($level, $message, ...$args)
    {
        if ($level <= static::$level) {
            ...
            $time = static::withColor(Time::now()->toString(' (YYYY-MM-dd HH:mm:ss) '), 'green');
			...
			if ($color = static::getLogColor($level)) {
				$_line = static::withColor('['.static::getLogName($level).']', $color).$time.static::withColor($line, 'magenta');
			}
    ...
    public static function reset()
    {
        ...
        Environment::current()->onOutput(function ($output) {
            $out = Stream::of('php://stdout');
            $out->write(str::encode($output, 'UTF-8'));
        });
    }
